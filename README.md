...
Далее нужно получить открытый ключ шифрования `YANDEX-DISK-KEY.GPG` из репозитария Яндекс Диска, после чего добавить полученный ключ в список надёжных репозитариев. Эти две консольные команды показаны в лист.4.2.
...
> [!NOTE]
> _**Примечание:**_
> Команда `apt-key` в настоящее время устарела, поскольку её действие носит глобальный характер, что может сказаться на безопасности, однако её использование не запрещено. Ожидается, что в будующем эта компнда будет заменена.

Следующим действие нужно выполнить создание списка sourse.list.d и обновить список пакетов во всех зарегистрированных в системе репозитариях. Это осуществляется командами, показанными в лист. 4.3.
...
Теперь всё готово для установки консольного клиента Яндекс Диска. Это выполняется одной командой (строка 1 лист. 4.4), после чего можно выполнить выход из режима суперпользователя (строка 2 лист.4.4) выполнив команду `exit`.
...
### 4.3. Выполнить настройки подключаемого аккаунта

Выполнение настроек подключаемого Яндекс Диска производим командой лист. 4.5. При этом, для авторизации необходимо воспользоваться веб-браузером на любом устройстве (рис. 4.3).
...
Команда `yandex-disk setup` запустит утилиту настройки (рис. 4.2). Утилита поставит перед пользователем ряд мнтерактивных вопросов. Если в ответ на эти вопросы ничего не вводить, то будут приняты значения по умолчанию. Вопросы утилиты настройки такие:
- Использовать прокси-сервер [y/N]? (значение по умолчанию НЕТ);
- Выполнить авторизацию в браузере с нужным аккаунтом и предложенным кодом (рис. 4.3) в течении ограниченного временного интервала;
...
_![Форма авторизации в браузере](https://github.com/tsf-soft/django-test-1/assets/6228605/8d96aeeb-36dc-4725-8b89-2dbf62291e56)
<p>Рисунок 4.3. Форма авторизации в браузере</p>_

При успешной авторизации в браузере, через который выполнялась авторизация, - будет отображена форма рис. 4.4, уведомляющая об успешно выполненной авторизации.
...
<p>Рисунок 4.4. Успешно пройденная авторизация</p>_

> [!NOTE]
> _**Примечание:**_
> _Авторизация в браузере будет произведена в текущем активном аккаунте Яндекс ID. Иначе говоря вы подключите тот экземпляр Яндекс Диска, который привязан к вашему текущему аккаунту. Если вам нужен другой экземпляр Яндекс Диска, то измените текущий активнй аккаунт путём авторизации в нём.


### 4.4. Оценить полученный вариант

Просмотр содержимого подлюченного сетевого диска с помощью команды лист. 4.6, показан на рис. 4.2.
...
На рис. 4.2 видно, что содержимое подключенного сетевого диска такое-же, что и содержимое этого же диска, подключенного для варианта <a href="#fig25">OS Windows</a> и для варианта  <a href="#fig35">OS Linux Ubuntu GUI</a>.

Следует отметить, что консольный клиент позволяет подкличить одновременно много экземпляров Яндекс Диска, каждый из которых будет принадлежать различным зарегистрированным аккаунтам Яндекс ID.

На рис. 4.5 показан вариант, в котором подключено 4 экземпляра Яндекс Диска. Они представлены следующими каталогами: Y1, Y2, Yandex.Disk, YY. Для каталога Yandex.Disk дополнительно создана короткая символьная ссылка командой:
...
![2023-09-25_18-51-19](https://github.com/tsf-soft/django-test-1/assets/6228605/ddd675d6-6b8e-4161-99ef-26b1af54aacc)
<p>Рисунок 4.5. Несколько подключенных аккаунтов Яндекс Диска</p>_
