...
Имеется несколько наиболее популярных интерпретаторов командной строки, при этом в каждой операционной системе как правило свой набор приложений командных интерпретаторов. В Linux наиболее используемым являетя интерпретатор [Bash](https://ru.wikipedia.org/wiki/Bash), в Windows это [PowerShell](https://ru.wikipedia.org/wiki/PowerShell) в macOS [Zsh](https://ru.wikipedia.org/wiki/Zsh), при этом версии командного интерпретатора **Bash**, имеются во всех операционных системах.

В этом разделе мы реализуем подключение Яндекс Диска в качестве сетевого диска к компьютеру с операционной системе Linux Ubuntu без графической оболочки (серверный вариант) используя командный интерпретатор **Bash**. Работа будет проведена путём подключения к Linux-машине по [SSH](https://ru.wikipedia.org/wiki/SSH). Последовательность шагов для достижения указанной цели следующая:

### 4.1. Подключиться к Linux-машине

Для подключения к Linux-машине, на удалённом компьютере нужно запустить терминал Bash. Демонстрируемый здесь командный интерпретатор входит в утилиту MobaXterm, которая в т.ч. реализована в OS Windows. После чего войти в Linux Ubuntu с аккаунтом зарегистрированного в системе пользователя. На рис. 4.1 показано окно терминала Bash, подключенного к Linux-машине. Интерпретатор ожидает ввода команд.
...
### 4.1. Выполнить консольные команды подключения

Прежде всего перейдём в режим пользователя с полномочиями суперпользователя (root). Для этого введём в терминале:

```bash

sudo su
[sudo] password for tsf:
root@linux-tsf:/home/tsf#
