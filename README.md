...
Для подключения к Linux-машине, на удалённом компьютере нужно запустить терминал Bash. После чего войти в Linux Ubuntu с аккаунтом зарегистрированного в системе пользователя. На рис. 4.1 показано окно терминала Bash, подключенного к Linux-машине. Интерпретатор ожидает ввода команд.

> [!NOTE]
> _**Примечание:**
> Демонстрируемый здесь командный интерпретатор является составной частью утилиты MobaXterm, которая в т.ч. реализована в OS Windows. Для достижения этого же результата можно воспользоваться иным командным интерпретатором.

<a name="fig41"></a>

![Терминал Bash](https://github.com/tsf-soft/django-test-1/assets/6228605/206da15d-889a-4ad8-a611-18781bf1e8ec)
_<p>Рисунок 3.7. Консольный клиент, подключеннй к Linux-машине</p>_

### 4.1. Выполнить консольные команды подключения Яндекс Диска

Прежде всего перейдём в режим пользователя с полномочиями суперпользователя (root). Для этого введём в терминале команду `sudo su` (строка 1 лист. 4.1):
...
> - промпт `root@linux-tsf:/home/tsf#` показывающий, что пользователь (tsf) находится в текущем каталоге `/home/tsf`, на компьютере с именем: `linux-tsf:` и имеет полномочия администратора (`root`);</br>
> 

Командный интерпретатор затребует пароль пользователя под которым вы вошли в систему (Лист. 4.1). Нужно ввести пароль пользователя помня, что в Unix системах, разновидностью которых является Linux Ubuntu - при вводе пароля, в окне терминала ничего не отображается (не отображаются даже символы заполнители).
