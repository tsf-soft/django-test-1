...
- чекбокс `Запомнить учетные данные` - отвечает за автоматическое последующее заполнение первых двух полей введёнными вами данными, в случае его выбора; значение по умолчанию false (не выбрано).
